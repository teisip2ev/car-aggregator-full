...
MAKES = {
    'Audi': 2, 'BMW': 4, 'Ford': 7, 'Honda': 1, 'Hyundai': 34,
    'Jeep': 32, 'Kia': 25, 'Land Rover': 42, 'Lexus': 35, 'Mazda': 6,
    'Mercedes-Benz': 12, 'Mitsubishi': 3, 'Nissan': 11, 'Opel': 5,
    'Peugeot': 16, 'Porsche': 140, 'Renault': 19, 'Skoda': 40,
    'Subaru': 23, 'Suzuki': 41, 'Tesla': 642, 'Toyota': 13,
    'Volkswagen': 8, 'Volvo': 10
}

supabase = create_client(SUPABASE_URL, SUPABASE_KEY)

def parse_listing(text, href, style=''):
    image_url = None
    if style:
    ...
    for line in lines:
        ...
        if price_match and listing["price_eur"] is None:
            try:
                ...
                pass
            continue
        if re.fullmatch(r"20[0-2]\d", line):
        ...
        if "km" in line:
            ...
            if km_match:
                try:
                    ...
                    pass
                continue
        if line in ["Diisel", "Bensiin", "Hübriid", "Elekter", "Gaasbensiin"]:
            listing["fuel"] = line
            continue
        if line in ["Automaat", "Käsitsi"]:
            listing["transmission"] = line
            continue
        if line in ["Sedaan", "Universaal", "Luukpära", "Maastur", "Kupee", "Kabriolett", "Minivan"]:
            listing["body"] = line
            continue
    ...
    while True:
        ...
        for r in results:
            if len(r["text"].strip()) > 20:
                parsed = parse_listing(r["text"], r["href"], r.get("image", ""))
                if parsed["price_eur"]:
